...
function SualRedakte() {
    ...
    // **Updated initializeKombinasiyaSual Function**
    const initializeKombinasiyaSual = (answersFromAPI) => {
        ...
        answersFromAPI.forEach((ans) => {
            ans.values.forEach((valObj) => {
                ...
                if (!optionsMap.has(val)) {
                    optionsMap.set(val, {
                        id: valObj.id, // Use the id from the API
        ...
        // Step 2: Map each question to its selected options using labels
        const questions = answersFromAPI.map((ans, index) => {
            const questionText = parseHtmlString(ans.key)[0] || `Sual ${index + 1}`;

            const selectedOptions = ans.values.map((valObj) => {
            ...
            return {
                id: ans.id, // Use the id from the API for the question
                idInApi: ans.id, // Assuming each question has an idInApi
        ...
        if (data.type_id === 1) {
            ...
            kombinasiyaQuestions.forEach((question) => {
                ...
                question.selectedOptions.forEach((label) => {
                    ...
                    if (option) {
                        if (option.id) { // Use `id` instead of `idInApi` for options
