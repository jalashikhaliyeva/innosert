  ...
  <section className="my-28">
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      {organizedCategories.map((category) => (
        <div
          key={category.id}
          className="font-gilroy p-6 bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300"
        >
          <h2 className="text-2xl font-medium mb-4">
            <Link
              href={`/imtahanlar/${encodeURIComponent(category.slug)}`}
              className="text-gray-800 hover:text-textHoverBlue transition-colors duration-300"
            >
              {category.name}
            </Link>
          </h2>
            ...
            <ul className="list-none pl-0">
              {category.subcategories.map((sub) => (
                <li key={sub.id} className="mb-2">
                  <Link
                    href={`/imtahanlar/${encodeURIComponent(category.slug)}/${encodeURIComponent(sub.slug)}`}
                    className="text-gray-600 hover:text-textHoverBlue transition-colors duration-300"
                  >
                    {sub.name}
                  </Link>
                </li>
