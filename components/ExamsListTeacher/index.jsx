...
const ExamsListTeacher = ({
  ...
  // Combine folders and exams into one array for easy sorting and display.
  const combinedItems = useMemo(() => {
    // If you have folders too, keep them; if not, you can omit or adapt.
    ...
    const examsList = (exams?.exams || exams || []).map((exam) => ({
      ...exam,
      type: "exam",
  ...
  // Sort the items according to the given sortOption.
      ...
      {viewMode === "grid" ? (
        <div className="grid grid-cols-2 gap-6 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
          {sortedItems.map((item, index) => {
            // Check if user is the author for exam items
            const userIsAuthor =
              item.type === "exam" ? isExamOwner(item.author) : true;
            ...
            return (
              ...
              >
                <div className="flex w-full justify-between items-center mb-4">
                  {/* Only show the checkbox if user is author AND item.type = exam */}
                  {item.type === "exam" && userIsAuthor && (
                    <input
                      type="checkbox"
                  ...
                  {/* More icon (edit, delete, etc.) - only shown if user is exam author */}
                  {userIsAuthor && (
                    <IoMdMore
                  ...
                  {dropdownVisible === index && userIsAuthor && (
                    <div
                      ref={dropdownRef}
                ...
                >
                          ...
                          <div className="relative group text-arrowButtonGray font-gilroy !text-base cursor-pointer">
                            <span className="truncate max-w-[15ch]">
                              {"Bu qovluqda imtahan yoxdur.".length > 17
                                ? "Bu qovluqda imtahan yoxdur.".substring(
                                    0,
                                    17
                                  ) + "..."
                  ...
                  {/* Folders: show date. Exams: show teacher if needed. */}
                  {item.type === "folder" && (
            ...
            return (
              ...
              >
                <a className="flex items-center w-full">
                  {/* Show checkbox only if item is exam AND user is the author */}
                  {item.type === "exam" && userIsAuthor && (
                    <input
                      type="checkbox"
                      className="w-4 h-4 mr-3 cursor-pointer appearance-none border-2 border-inputBorder rounded checked:bg-inputBorder checked:border-inputBorder checked:before:content-['✔'] checked:before:text-white checked:before:block checked:before:text-center checked:before:leading-none checked:before:text-xs focus:outline-none"
                  ...
                  <div className="flex items-center w-full justify-between">
                    ...
                    {/* More icon (dropdown) - only if user is the exam's author */}
                    {userIsAuthor && (
                      <div
                        onClick={(e) => {
                          e.stopPropagation();
